...
"            </div>\n"
"            <div style=\"margin: 10px 0;\">\n"
"                Temperature thresholds: <span id=\"temp-low-display\">--</span>°C to <span id=\"temp-high-display\">--</span>°C\n"
"            </div>\n"
"        </div>\n"
"        \n"
"        <div class=\"card\">\n"
...
"            <label>Low (°C): </label>\n"
"            <input type=\"number\" id=\"temp-low\" style=\"width: 60px;\" min=\"10\" max=\"30\">\n"
"        </div>\n"
"        <div>\n"
"            <label>High (°C): </label>\n"
"            <input type=\"number\" id=\"temp-high\" style=\"width: 60px;\" min=\"10\" max=\"30\">\n"
"        </div>\n"
"    </div>\n"
...
"        dataInterval = setInterval(updateAllData, 500);\n"
"        updateAllData();\n"
"        \n"
...
"                    \n"
"                    // Temperature thresholds (parts[8] and parts[9])\n"
"                    document.getElementById('temp-low-display').textContent  = parts[8];\n"
"                    document.getElementById('temp-high-display').textContent = parts[9];\n"
"                    document.getElementById('temp-low').value              = parts[8];\n"
"                    document.getElementById('temp-high').value             = parts[9];\n"
"                    \n"
...
"            const lowTemp  = document.getElementById('temp-low').value;\n"
"            const highTemp = document.getElementById('temp-high').value;\n"
"            if (!lowTemp || !highTemp) {\n"
"                alert('Please enter both threshold values');\n"
"                return;\n"
"            }\n"
...
"            \n"
"            fetch(`/threshold?low=${lowTemp}&high=${highTemp}`)\n"
"                .then(response => response.text())\n"
"                .then(data => {\n"
"                    alert(data);\n"
"                    updateClimateData();\n"
...
static esp_err_t data_handler(httpd_req_t *req)
{
    ...
    // Format response with all system data including thresholds
    // Format: temp,humidity,fan,heater,rfid_uid,rfid_auth,nrf_mode,nrf_message,temp_low,temp_high

    int written = snprintf(resp, sizeof(resp), "%d,%d,%d,%d,%s,%d,%d,%s,%d,%d",
             ...
             strlen(last_nrf_message) > 0 ? last_nrf_message : "None",
             temp_low,
             temp_high);
...
static esp_err_t threshold_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        // Parse low and high parameters
        if (httpd_query_key_value(buf, "low", low_str, sizeof(low_str)) == ESP_OK &&
            httpd_query_key_value(buf, "high", high_str, sizeof(high_str)) == ESP_OK) {

            int low = atoi(low_str);
            int high = atoi(high_str);

            ESP_LOGI(TAG, "Threshold update request: low=%d, high=%d", low, high);

            if (low < high) {
                // Update thresholds using the function from climate_control.c
                set_temperature_thresholds(low, high);
                sprintf(resp, "Temperature thresholds updated - Low: %d°C, High: %d°C", low, high);
            } else {
                strcpy(resp, "Invalid thresholds: low must be less than high");
            }
        } else {
            strcpy(resp, "Missing threshold parameters");
        }
    } else {
...
void start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &nrf_send);
         ...
         httpd_uri_t threshold = {
            .uri = "/threshold",
            .method = HTTP_GET,
            .handler = threshold_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(server, &threshold);
