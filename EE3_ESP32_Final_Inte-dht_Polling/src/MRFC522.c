/* REFERENCE
 * https://github.com/blmhemu/ESP32-MFRC522
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
uint8_t PCD_ReadRegister(spi_device_handle_t spi, uint8_t Register)
{
	...
	t.tx_buffer = &reg;
	t.rx_buffer = &val;
	gpio_set_level(RFID_PIN_CS, 0);
	ret = spi_device_transmit(spi, &t);
	assert(ret == ESP_OK);
	t.tx_buffer = (uint8_t *)0;
	ret = spi_device_transmit(spi, &t);
	gpio_set_level(RFID_PIN_CS, 1);
...
void PCD_Init(spi_device_handle_t spi)
{
	...
	PCD_Version(spi);
	//  gpio_pad_select_gpio(RFID_PIN_RST);
...
{
	char uidString[32] = {0}; // Buffer to store UID as a string
	int offset = 0;			  // Offset to keep track of where we are in the string
	...
	for (uint8_t i = 0; i < uid->size; i++)
	{
		...
		// Add the byte to the UID string
		offset += snprintf(uidString + offset, sizeof(uidString) - offset, "%02x", uid->uidByte[i]);
	...
	printf("UID as variable: %s\n", uidString); // Here you can use uidString as the variable
	...
	{
	case PICC_TYPE_MIFARE_1K:
		printf("PICC type: MIFARE 1KB\n");
		break;
	case PICC_TYPE_MIFARE_4K:
		printf("PICC type: MIFARE 4KB\n");
		break;
	case PICC_TYPE_MIFARE_UL:
		printf("PICC type: MIFARE Ultralight\n");
		break;
	case PICC_TYPE_MIFARE_MINI:
		printf("PICC type: MIFARE Mini\n");
		break;
	default:
		printf("PICC type: Unknown\n");
		break;
	}

	// Now you have the UID as a string in `uidString` and can use it elsewhere if needed
}
